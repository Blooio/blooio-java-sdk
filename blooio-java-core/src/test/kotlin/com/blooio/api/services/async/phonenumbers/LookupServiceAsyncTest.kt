// File generated from our OpenAPI spec by Stainless.

package com.blooio.api.services.async.phonenumbers

import com.blooio.api.client.okhttp.BlooioOkHttpClientAsync
import com.blooio.api.models.phonenumbers.lookup.LookupCreateParams
import com.blooio.api.models.phonenumbers.lookup.LookupRetrieveParams
import org.junit.jupiter.api.Disabled
import org.junit.jupiter.api.Test

internal class LookupServiceAsyncTest {

    @Disabled("Mock server tests are disabled")
    @Test
    fun create() {
        val client = BlooioOkHttpClientAsync.builder().apiKey("My API Key").build()
        val lookupServiceAsync = client.phoneNumbers().lookup()

        val phoneNumberLookupResultFuture =
            lookupServiceAsync.create(LookupCreateParams.builder().number("+12125551234").build())

        val phoneNumberLookupResult = phoneNumberLookupResultFuture.get()
        phoneNumberLookupResult.validate()
    }

    @Disabled("Mock server tests are disabled")
    @Test
    fun retrieve() {
        val client = BlooioOkHttpClientAsync.builder().apiKey("My API Key").build()
        val lookupServiceAsync = client.phoneNumbers().lookup()

        val phoneNumberLookupResultFuture =
            lookupServiceAsync.retrieve(
                LookupRetrieveParams.builder().number("+12125551234").build()
            )

        val phoneNumberLookupResult = phoneNumberLookupResultFuture.get()
        phoneNumberLookupResult.validate()
    }
}
