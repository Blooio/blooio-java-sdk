...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { BlooioInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw BlooioInvalidDataException("`$name` is not set")
            is JsonNull -> throw BlooioInvalidDataException("`$name` is null")
            else -> throw BlooioInvalidDataException("`$name` is invalid, received $this")
        }
